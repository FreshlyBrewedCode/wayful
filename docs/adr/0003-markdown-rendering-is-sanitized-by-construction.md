...
The UI renders markdown it did not write — artifact content, step bodies, goal bodies and type
...
disclosure, not a defacement. The UI uses `react-markdown` with `remark-gfm` specifically
...
- **No syntax highlighting** in the first cut. Shiki and Prism are large, and the UI bundle is
  embedded into the CLI binary.

## Consequences

One renderer serves every markdown surface in the UI, which retires the note in
