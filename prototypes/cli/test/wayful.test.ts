...
const entrypoint = join(cliRoot, "wayful");
...
function invoke(args: string[], cwd: string, environment: Record<string, string> = {}) {
  const result = Bun.spawnSync({
    cmd: [entrypoint, ...args], cwd, env: { ...process.env, ...environment }, stdout: "pipe", stderr: "pipe",
  });
  return { exitCode: result.exitCode, stderr: decoder.decode(result.stderr), stdout: decoder.decode(result.stdout) };
}
...
async function writeFakeBun(directory: string, version: string) {
  const fakeBun = join(directory, "bun");
  await writeFile(fakeBun, `#!/bin/sh\nif [ "$1" = "--version" ]; then echo "${version}"; exit 0; fi\nprintf '%s\\n' "$@" > "$WAYFUL_FAKE_BUN_ARGS"\n`);
  await chmod(fakeBun, 0o755);
}

/** Creates documented on-disk input only; it never uses the CLI under test. */
async function projectFixture(options: { map?: string; type?: string; typeBody?: string; typeSlots?: string; mapFields?: string } = {}) {
...
describe("the wayful executable entrypoint", () => {
  test("reports an actionable diagnostic when Bun is absent", async () => {
    const directory = await temporaryDirectory();
    const result = invoke(["--help"], directory, { PATH: "" });
    expect(result.exitCode).toBe(2);
    expect(result.stderr).toContain("wayful:");
    expect(result.stderr).toContain("Bun 1.4.1 or later is required");
  });

  test("rejects an unsupported Bun version using the package engine", async () => {
    const directory = await temporaryDirectory();
    await writeFakeBun(directory, "1.1.99");
    const result = invoke(["--help"], directory, { PATH: `${directory}:/usr/bin:/bin`, WAYFUL_FAKE_BUN_ARGS: join(directory, "args") });
    expect(result.exitCode).toBe(2);
    expect(result.stderr).toContain("1.4.1 or later is required");
    expect(result.stderr).toContain("found 1.1.99");
  });

  test.each(["1.3.99", "1.4.0", "1.4.1-canary.1", "1.4.2-", "01.4.2", "1.4.2-01", "1.4.2-alpha.01"])("rejects Bun %s when it is below the package engine or malformed", async (version) => {
    const directory = await temporaryDirectory();
    await writeFakeBun(directory, version);
    const result = invoke(["--help"], directory, { PATH: `${directory}:/usr/bin:/bin`, WAYFUL_FAKE_BUN_ARGS: join(directory, "args") });
    expect(result.exitCode).toBe(2);
    expect(result.stderr).toContain("1.4.1 or later is required");
  ...
  test("dispatches through Bun with the TypeScript program followed by unchanged arguments", async () => {
    const directory = await temporaryDirectory();
    const argumentsFile = join(directory, "args");
    await writeFakeBun(directory, "1.4.1");
    expect(invoke(["map", "show", "--map", "redesign", "--json"], directory, { PATH: `${directory}:/usr/bin:/bin`, WAYFUL_FAKE_BUN_ARGS: argumentsFile }).exitCode).toBe(0);
    const dispatched = (await readFile(argumentsFile, "utf8")).trim().split("\n");
    expect(dispatched[0]).toBe(join(cliRoot, "src", "main.ts"));
    expect(dispatched.slice(1)).toEqual(["map", "show", "--map", "redesign", "--json"]);
  });
});
...
describe("project and context contracts", () => {
  ...
  test("documents each command's required arguments and available flags in help", async () => {
    ...
    for (const [args, expected] of cases) {
      const result = invoke(args as string[], project);
      expect(result.exitCode).toBe(0);
  ...
  test("init creates versioned metadata and the generic task type, then refuses overwrite", async () => {
    ...
    expect(await readFile(join(project, ".wayful", "project.toml"), "utf8")).toContain("format_version = 1");
    expect(await readFile(join(project, ".wayful", "types", "task.md"), "utf8")).toContain("name: task");
  ...
  test("discovers upward, honors WAYFUL_PROJECT, and lets --project override it", async () => {
    ...
    expect(invoke(["map", "show", "--map", "other", "--json"], nested, { WAYFUL_PROJECT: other }).exitCode).toBe(0);
    expect(invoke(["map", "show", "--project", project, "--map", "plan", "--json"], nested, { WAYFUL_PROJECT: other }).exitCode).toBe(0);
  });
  ...
  test("rejects absent, malformed, and newer project metadata plus unsafe identifiers", async () => {
    ...
    expectCommandError(invoke(["type", "list"], project));
    expectCommandError(invoke(["map", "create", "--map", "../unsafe", "--start", "now", "--goal", "done"], project));
  });
  ...
  test("strictly validates project metadata while accepting the documented unversioned migration form", async () => {
    ...
    for (const invalid of ["format_version = 1\ndescription = 42\n", "format_version = 1\n", "format_version = 1\ndescription = \"ok\"\nunknown = true\n", "format_version = \"1\"\ndescription = \"ok\"\n"]) {
  ...
  test("requires explicit map context and gives --map precedence over WAYFUL_MAP", async () => {
    ...
    expect(invoke(["map", "show", "--map", "plan", "--json"], project, { WAYFUL_MAP: "other" }).exitCode).toBe(0);
  });
});
...
describe("maps, types, and readable rendering", () => {
  test("writes generated YAML collections in multiline block style", async () => {
    ...
    expect(invoke(["map", "create", "--map", "plan", "--start", "here", "--goal", "done", "--goal-body", "Completion notes."], project).exitCode).toBe(0);
    expect(invoke(["artifact", "add", "proof", "--map", "plan", "--kind", "document", "--ref", "# opaque: value"], project).exitCode).toBe(0);
    expect(invoke(["step", "create", "work", "--map", "plan", "--type", "task", "--description", "Do it", "--required-inputs", "[{\"name\":\"brief\",\"kind\":\"document\"}]"], project).exitCode).toBe(0);
    ...
    expect(await readFile(generated[2], "utf8")).toContain("dependencies: \n  []\ninputs: \n  []\noutputs: \n  []\n");
    expect(await readFile(generated[2], "utf8")).toContain("required_inputs: \n  - name: brief\n    kind: document\n");
  ...
  test("creates maps with required fields, no overwrite, and deterministic JSON read views", async () => {
    ...
    expectCommandError(invoke(["map", "create", "--map", "plan", "--start", "now", "--goal", "done"], project));
  ...
  test("emits structured, read-only validation results for valid and invalid maps", async () => {
    ...
    expect(JSON.parse(invalid.stdout)).toEqual({ valid: false, errors: ["unsupported format version in map metadata."] });
...
describe("steps and graph integrity", () => {
  test("creates, updates, preserves, and shows an optional step body", async () => {
    const project = await projectFixture();
    expect(invoke(["step", "create", "research", "--map", "plan", "--type", "task", "--description", "Research users", "--body", "Initial notes.\nSecond line."], project).exitCode).toBe(0);

    let shown = invoke(["step", "show", "research", "--map", "plan", "--json"], project);
    expect(JSON.parse(shown.stdout).body).toBe("Initial notes.\nSecond line.");
    expect(invoke(["step", "show", "research", "--map", "plan"], project).stdout).toContain("Initial notes.\nSecond line.");

    expect(invoke(["step", "update", "research", "--map", "plan", "--body", "Revised notes."], project).exitCode).toBe(0);
    ...
    expect(JSON.parse(invoke(["map", "show", "--map", "plan", "--json"], project).stdout).steps[0].body).toBe("Revised notes.");
  ...
  test("creates typed steps with monotonic IDs, persists frontmatter, and resolves name or ID", async () => {
    const project = await projectFixture();
    expect(invoke(["step", "create", "research", "--map", "plan", "--type", "task", "--description", "Research users"], project).exitCode).toBe(0);
    expect(invoke(["step", "create", "design", "--map", "plan", "--type", "task", "--description", "Design it"], project).exitCode).toBe(0);
    expect(await readFile(join(project, ".wayful", "maps", "plan", "steps", "1-research.md"), "utf8")).toContain("id: 1");
    expect(invoke(["step", "show", "1", "--map", "plan", "--json"], project).exitCode).toBe(0);
    expect(invoke(["step", "show", "research", "--map", "plan", "--json"], project).exitCode).toBe(0);
  });

  test("versions created steps and rejects missing, malformed, or unsupported step versions", async () => {
    const project = await projectFixture();
    expect(invoke(["step", "create", "work", "--map", "plan", "--type", "task", "--description", "Do it"], project).exitCode).toBe(0);
  ...
  test("updates pending descriptions without losing Markdown body and rejects numeric step names", async () => {
    ...
    expect(invoke(["step", "update", "research", "--map", "plan", "--description", "Updated"], project).exitCode).toBe(0);
    expect(await readFile(join(project, ".wayful", "maps", "plan", "steps", "1-research.md"), "utf8")).toContain("Narrative that must survive");
    expectCommandError(invoke(["step", "create", "123", "--map", "plan", "--type", "task", "--description", "Invalid"], project));
  });
  ...
  test("enforces reasons, allowed transitions, terminal immutability, and completion summaries", async () => {
    ...
    expect(invoke(["step", "block", "work", "--map", "plan", "--reason", "Waiting"], project).exitCode).toBe(0);
    expect(invoke(["step", "unblock", "work", "--map", "plan"], project).exitCode).toBe(0);
    expect(await readFile(join(project, ".wayful", "maps", "plan", "steps", "1-work.md"), "utf8")).not.toContain("block_reason");
    expectCommandError(invoke(["step", "complete", "work", "--map", "plan", "--summary", ""], project));
    expect(invoke(["step", "cancel", "work", "--map", "plan", "--reason", "Obsolete"], project).exitCode).toBe(0);
    expectCommandError(invoke(["step", "update", "work", "--map", "plan", "--description", "Changed"] , project));
  });
  ...
  test("rejects self, duplicate, cyclic, cancelled, and completed-step dependencies", async () => {
    ...
    expect(invoke(["step", "depends", "a", "--map", "plan", "--on", "b"], project).exitCode).toBe(0);
...
describe("artifacts, goals, and validation", () => {
  test("creates, preserves, and shows optional goal bodies", async () => {
    const project = await projectFixture();
    expect(invoke(["goal", "add", "--map", "plan", "--name", "release", "--description", "Approved", "--body", "Acceptance details.\nSecond line."], project).exitCode).toBe(0);

    const listed = invoke(["goal", "list", "--map", "plan", "--json"], project);
    expect(JSON.parse(listed.stdout)[0].body).toBe("Acceptance details.\nSecond line.");
    expect(invoke(["goal", "list", "--map", "plan"], project).stdout).toContain("Acceptance details.\n  Second line.");

    expect(invoke(["artifact", "add", "proof", "--map", "plan", "--kind", "document", "--ref", "git:abc"], project).exitCode).toBe(0);
    expect(invoke(["goal", "satisfy", "--map", "plan", "--goal", "release", "--artifact", "proof"], project).exitCode).toBe(0);
    const map = invoke(["map", "show", "--map", "plan", "--json"], project);
    expect(JSON.parse(map.stdout).goals[0].body).toBe("Acceptance details.\nSecond line.");
    expect(invoke(["map", "show", "--map", "plan"], project).stdout).toContain("Acceptance details.\n    Second line.");
    expect(await readFile(join(project, ".wayful", "maps", "plan", "goals", "release.md"), "utf8")).toContain("Acceptance details.\nSecond line.");
  });

  test("registers opaque artifacts once and attaches them by explicit matching slots or supplementarily", async () => {
    const project = await projectFixture({ typeSlots: "required_inputs:\n  - name: brief\n    kind: document\nrequired_outputs:\n  - name: report\n    kind: document\n" });
    await writeStep(project, "work", 1, "required_inputs:\n  - name: brief\n    kind: document\nrequired_outputs:\n  - name: report\n    kind: document\n");
    expect(invoke(["artifact", "add", "proof", "--map", "plan", "--kind", "document", "--ref", "git:abc"], project).exitCode).toBe(0);
    expectCommandError(invoke(["artifact", "add", "proof", "--map", "plan", "--kind", "document", "--ref", "git:def"], project));
    expect(invoke(["step", "input", "work", "--map", "plan", "--artifact", "proof", "--slot", "brief"], project).exitCode).toBe(0);
    expect(invoke(["step", "output", "work", "--map", "plan", "--artifact", "proof"], project).exitCode).toBe(0);
    expectCommandError(invoke(["step", "complete", "work", "--map", "plan", "--summary", "Missing required output"], project));
    expect(invoke(["step", "output", "work", "--map", "plan", "--artifact", "proof", "--slot", "report"], project).exitCode).toBe(0);
    expect(invoke(["step", "complete", "work", "--map", "plan", "--summary", "Evidence recorded"], project).exitCode).toBe(0);
    expectCommandError(invoke(["step", "input", "work", "--map", "plan", "--artifact", "proof", "--slot", "report"], project));
  });

  test("refuses an artifact name already represented by a .yml record", async () => {
    const project = await projectFixture();
    await writeFile(join(project, ".wayful", "maps", "plan", "artifacts", "proof.yml"), "format_version: 1\nname: proof\nkind: document\nref: git:one\n");
    expectCommandError(invoke(["artifact", "add", "proof", "--map", "plan", "--kind", "document", "--ref", "git:two"], project));
    await expect(readFile(join(project, ".wayful", "maps", "plan", "artifacts", "proof.yaml"), "utf8")).rejects.toThrow();
  });

  test("versions created artifacts and rejects missing, malformed, or unsupported artifact versions", async () => {
    const project = await projectFixture();
    expect(invoke(["artifact", "add", "proof", "--map", "plan", "--kind", "document", "--ref", "git:one"], project).exitCode).toBe(0);
  ...
  test("adds, lists, and satisfies named goals only with existing evidence and no re-satisfaction", async () => {
    const project = await projectFixture();
    expectCommandError(invoke(["goal", "add", "--map", "plan", "--description", "Missing name"], project));
    expect(invoke(["goal", "add", "--map", "plan", "--name", "release", "--description", "Approved"], project).exitCode).toBe(0);
    expect(invoke(["goal", "list", "--map", "plan", "--json"], project).exitCode).toBe(0);
    expectCommandError(invoke(["goal", "satisfy", "--map", "plan", "--goal", "release", "--artifact", "missing"], project));
    expect(invoke(["artifact", "add", "approval", "--map", "plan", "--kind", "document", "--ref", "pr:42"], project).exitCode).toBe(0);
    expect(invoke(["goal", "satisfy", "--map", "plan", "--goal", "release", "--artifact", "approval"], project).exitCode).toBe(0);
    expectCommandError(invoke(["goal", "satisfy", "--map", "plan", "--goal", "release", "--artifact", "approval"], project));
  });
  ...
  test("computes sorted actionable next work and validates without mutation using documented exit codes", async () => {
    ...
    expect(invoke(["map", "validate", "--map", "plan"], project).exitCode).toBe(1);
    expect(await readFile(join(project, ".wayful", "maps", "plan", "map.toml"), "utf8")).toBe(before);
  });

  test("reports malformed persisted frontmatter as an invalid map rather than modifying it", async () => {
    const project = await projectFixture();
    await writeFile(join(project, ".wayful", "maps", "plan", "steps", "1-bad.md"), "---\nname: bad\n");
  ...
  test("reports null attachments, missing slot arrays, and missing lifecycle fields as invalid maps", async () => {
    const project = await projectFixture();
    await writeStep(project, "null-attachment", 1, "inputs: [null]\noutputs: []\nrequired_inputs:\n  - name: source\n    kind: document\nrequired_outputs: []\n");
    ...
    await writeFile(stepFile, (await readFile(stepFile, "utf8")).replace("required_inputs:\n  - name: source\n    kind: document\n", ""));
    result = invoke(["map", "validate", "--map", "plan"], project);
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("required_inputs must be an array");

    await rm(stepFile);
    await writeStep(project, "complete-without-summary", 2, "status: complete\ndependencies: []\ninputs: []\noutputs: []\nrequired_inputs: []\nrequired_outputs: []\n");
    result = invoke(["map", "validate", "--map", "plan"], project);
    expect(result.exitCode).toBe(1);
  ...
  test("requires cancellation and block reasons in manually authored steps", async () => {
    const project = await projectFixture();
    await writeStep(project, "cancelled-without-reason", 1, "status: cancelled\ndependencies: []\ninputs: []\noutputs: []\nrequired_inputs: []\nrequired_outputs: []\n");
    let result = invoke(["map", "validate", "--map", "plan"], project);
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("cancellation reason is required");
    await writeFile(join(project, ".wayful", "maps", "plan", "steps", "1-cancelled-without-reason.md"), (await readFile(join(project, ".wayful", "maps", "plan", "steps", "1-cancelled-without-reason.md"), "utf8")).replace("status: cancelled", "status: blocked"));
    result = invoke(["map", "validate", "--map", "plan"], project);
    expect(result.exitCode).toBe(1);
  ...
  test("rejects unsupported options and extra positionals before mutations", async () => {
    ...
    for (const args of [
      ...
      ["step", "create", "new-work", "extra", "--map", "plan", "--type", "task", "--description", "Must not write"],
      ["artifact", "add", "proof", "extra", "--map", "plan", "--kind", "document", "--ref", "ref"],
      ["goal", "add", "--map", "plan", "--name", "release", "--description", "Released", "--unknown", "value"],
    ...
    await expect(readFile(join(project, ".wayful", "maps", "plan", "steps", "1-new-work.md"), "utf8")).rejects.toThrow();
    expect(invoke(["--project", project, "map", "show", "--map", "plan", "--json"], project).exitCode).toBe(0);
  });

  test("rejects dangling supplementary attachments and duplicate YAML artifact identities", async () => {
    const project = await projectFixture();
    await writeStep(project, "work", 1, "inputs:\n  - artifact: missing\noutputs: []\nrequired_inputs: []\nrequired_outputs: []\n");
    await writeFile(join(project, ".wayful", "maps", "plan", "artifacts", "proof.yaml"), "format_version: 1\nname: proof\nkind: document\nref: git:one\n");
    await writeFile(join(project, ".wayful", "maps", "plan", "artifacts", "proof.yml"), "format_version: 1\nname: proof\nkind: document\nref: git:two\n");
    const result = invoke(["map", "validate", "--map", "plan"], project);
    expect(result.exitCode).toBe(1);
  ...
  test("validates manually authored slot attachments in both directions", async () => {
    ...
    await writeFile(join(artifactDirectory, "document.yaml"), "format_version: 1\nname: document\nkind: document\nref: doc\n");
    await writeFile(join(artifactDirectory, "image.yaml"), "format_version: 1\nname: image\nkind: image\nref: image\n");
    const stepDirectory = join(project, ".wayful", "maps", "plan", "steps");
    const manualStep = (id: number, name: string, inputs: string) => writeFile(join(stepDirectory, `${id}-${name}.md`), `---\nformat_version: 1\nid: ${id}\nname: ${name}\ntype: task\ndescription: Manually edited\nstatus: pending\ndependencies: []\ninputs:\n${inputs}outputs: []\nrequired_inputs:\n  - name: source\n    kind: document\nrequired_outputs:\n  - name: report\n    kind: document\n---\n`);
    await manualStep(1, "unknown", "  - artifact: document\n    slot: absent\n");
    await manualStep(2, "wrong-direction", "  - artifact: document\n    slot: report\n");
    await manualStep(3, "duplicate", "  - artifact: document\n    slot: source\n  - artifact: document\n    slot: source\n");
  ...
  test("refuses every map mutation when existing map identities are globally invalid", async () => {
    ...
    await writeFile(join(mapDirectory, "artifacts", "existing.yaml"), "format_version: 1\nname: existing\nkind: document\nref: git:existing\n");
    ...
    for (const args of [
      ...
      ["step", "create", "new-work", "--map", "plan", "--type", "task", "--description", "Must not write"],
  ...
  test("renders deterministic human status with goals, counts, blockers, and actionable steps", async () => {
    const project = await projectFixture();
    expect(invoke(["goal", "add", "--map", "plan", "--name", "release", "--description", "Release it"], project).exitCode).toBe(0);
    ...
    await writeFile(waiting, (await readFile(waiting, "utf8")).replace("status: pending", "status: blocked\nblock_reason: Awaiting approval"));
    const result = invoke(["map", "status", "--map", "plan"], project);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe(["Map plan", "Goals: 0/1 satisfied", "Steps: 1 pending, 1 blocked, 0 complete, 0 cancelled", "Blockers:", "- 2 waiting: Awaiting approval", "Actionable steps:", "- 1 ready: Original description", ""].join("\n"));
  });
});
