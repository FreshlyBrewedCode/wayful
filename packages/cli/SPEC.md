...
45. As a user, I want a `ui` command that serves the Wayful UI for a project, so that I can read a map as a graph, board, or list without assembling a web server myself.
46. As a user, I want `wayful ui` to carry its own client, so that viewing a project needs neither a checkout of the UI sources nor a separate build step at use time.
47. As a UI developer, I want a `serve` command that exposes only the API, so that a development client can proxy to it while it is rebuilt independently.
...
- The server reads through the same `WayfulBackend` and `domain/` functions the commands use rather than re-deriving state or invoking the CLI as a subprocess, so its JSON is the CLI's JSON by construction. Its endpoints are total: a failure becomes an `error` field in the response body — the same `{"error": "<message>"}` shape `--json` emits — rather than a non-2xx status, so a UI can render a broken project instead of a blank page. It never writes.
...
- The UI client is built by `packages/ui` and copied into `packages/cli/dist/ui` by this package's `build` script, so an installed CLI serves a client it owns. `wayful ui` looks for `$WAYFUL_UI_DIST`, then that bundled copy, then a sibling `packages/ui/dist` workspace build, and fails with a build instruction when none holds an `index.html`. Requests that are not API paths serve a built asset when one matches and the SPA shell otherwise, so client-routed deep links work; a path resolving outside the client directory gets the shell, never the file.
