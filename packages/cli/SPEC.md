...
45. As a user, I want a `ui` command that serves the Wayful viewer for a project, so that I can read a map as a graph, board, or list without assembling a web server myself.
46. As a user, I want `wayful ui` to carry its own client, so that viewing a project needs neither a checkout of the viewer sources nor a separate build step at use time.
47. As a viewer developer, I want a `serve` command that exposes only the API, so that a development client can proxy to it while it is rebuilt independently.
48. As an automation author, I want `--project` to select what a running server reads, so that a server is bound to one project regardless of where it was launched or what the ambient environment says.
...
- `wayful ui` and `wayful serve` run a local read-only HTTP server until interrupted. Both accept `--port PORT` (default `7830`; `0` binds an ephemeral port) and `--host ADDR` (default `127.0.0.1`), and take project context from the shared `--project` flag, then `WAYFUL_PROJECT`, then the working directory. `serve` exposes only the JSON API; `ui` additionally serves the viewer client. Each prints a banner naming the resolved project root, the client directory when one is served, and the URL actually bound — which is the only way a `--port 0` caller learns its port.
- The server's endpoints are `GET /api/overview` (project metadata, every map with its `map status`, and the project types), `GET /api/map?name=` (`map show`, `map status`, `map next`, and `map validate` composed into one response), `GET /api/step?map=&ref=` (a step with its body and its type's current instructions), and `GET /api/events` (a Server-Sent Events stream that emits `hello` on connect and `changed` on any `.wayful` modification). Unknown `/api/` paths are 404 and never fall back to the client.
- The server reads through the same `WayfulBackend` and `domain/` functions the commands use rather than re-deriving state or invoking the CLI as a subprocess, so its JSON is the CLI's JSON by construction. Its endpoints are total: a failure becomes an `error` field in the response body — the same `{"error": "<message>"}` shape `--json` emits — rather than a non-2xx status, so a viewer can render a broken project instead of a blank page. It never writes.
- A running server resolves its project once per request from the directory it was started with, and never reads `WAYFUL_PROJECT` or `WAYFUL_MAP` again, so an ambient shell value cannot redirect it and `wayful init` in the served directory is picked up without a restart. A directory with no project still starts a server; the absence is reported in `/api/overview`.
- The viewer client is built by `packages/ui` and copied into `packages/cli/dist/ui` by this package's `build` script, so an installed CLI serves a client it owns. `wayful ui` looks for `$WAYFUL_UI_DIST`, then that bundled copy, then a sibling `packages/ui/dist` workspace build, and fails with a build instruction when none holds an `index.html`. Requests that are not API paths serve a built asset when one matches and the SPA shell otherwise, so client-routed deep links work; a path resolving outside the client directory gets the shell, never the file.
...
- Server tests start the real server in-process on an ephemeral port against a temporary project and assert over HTTP: every endpoint's payload, the `{error}` body for unreadable maps and missing steps, the SSE greeting and change notification, 404 for unknown API paths, project resolution from a nested directory and from `--project`, starting where no project exists, API-only versus client-serving modes, asset serving, SPA deep-link fallback, and that a traversing path cannot reach outside the client directory. Client-directory lookup is tested directly. `ui` and `serve` additionally get end-to-end coverage through a child Bun process started with `--port 0`, driven through the banner it prints.
...
- Mutating the project over HTTP, authentication, TLS, non-loopback deployment, or any server behavior beyond a single local reader.
