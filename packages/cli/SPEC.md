...
- The CLI is organized around one high-level executable seam: command invocation resolves context, loads/persists the filesystem model, applies domain validation, then renders output. Internal modules may separate argument parsing, persistence, domain operations, and rendering, but behavior is tested through the executable rather than private internals.
...
- Dependencies outside Bun's built-in APIs.
...
- The project currently has no established CLI code or test prior art. The process-level CLI suite is the initial, preferred test pattern for this subsystem.
