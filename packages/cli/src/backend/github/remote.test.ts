...
describe("resolveOriginRemote", () => {
  ...
  test("runs git in the requested directory", async () => {
    let cwd: string | undefined;
    const layer = stubChildProcessSpawner((_invocation, options) => {
      cwd = options.cwd;
      return Effect.succeed("");
    });
    await Effect.runPromise(resolveOriginRemote("/project/root").pipe(Effect.provide(layer)));
    expect(cwd).toBe("/project/root");
  });
});
