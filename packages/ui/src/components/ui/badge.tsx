  ...
  "inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs font-medium w-fit whitespace-nowrap shrink-0 gap-1 [&>svg]:size-3 [&>svg]:pointer-events-none transition-[color,box-shadow] overflow-hidden",
  {
    variants: {
      variant: {
        ...
        // Tinted from the `--status` variable set by `data-status`.
        status:
          "border-[color-mix(in_oklab,var(--status)_45%,transparent)] bg-[color-mix(in_oklab,var(--status)_16%,transparent)] text-[var(--status)]",
      },
    },
    defaultVariants: {
      variant: "default",
